...
def test_dashboard_hien_thi_hang_muc_tre_tien_do(toan_quyen):
    """Hạng mục ĐÃ khởi công (NgayKhoiCong ở quá khứ) và quá ngày dự kiến
    hoàn thành -> đúng là "trễ tiến độ", phải hiện trong cảnh báo."""
    khoi_cong = (date.today() - timedelta(days=10)).isoformat()
    qua_han = (date.today() - timedelta(days=3)).isoformat()
    toan_quyen.post('/hangmuc/them', data={'MaHangMuc': 'HM01', 'TenHangMuc': 'Công trình A',
                                             'NgayKhoiCong': khoi_cong,
...
def test_dashboard_khong_bao_tre_tien_do_khi_chua_ro_ngay_khoi_cong(toan_quyen):
    """LỖI CŨ: hạng mục CHƯA khai ngày khởi công (NgayKhoiCong bỏ trống -
    tức KHÔNG BIẾT hạng mục đã bắt đầu thi công hay chưa) nhưng lỡ có
    ThoiGianHoanThanhDuKien nằm trong quá khứ vẫn bị xếp NHẦM vào "trễ tiến
    độ"/"Quá hạn", trong khi thực ra không đủ dữ liệu để khẳng định điều
    đó (có thể còn chưa khởi công thật). Phải sửa để trường hợp này rơi
    đúng vào "Chưa xác định" thay vì "Quá hạn", và KHÔNG hiện trong cảnh
    báo trễ tiến độ."""
    qua_han = (date.today() - timedelta(days=3)).isoformat()
    toan_quyen.post('/hangmuc/them', data={'MaHangMuc': 'HM01', 'TenHangMuc': 'Công trình B',
                                             'ThoiGianHoanThanhDuKien': qua_han}, follow_redirects=True)
    r = toan_quyen.get('/dashboard/')
    text = r.data.decode('utf-8')
    assert 'không có hạng mục nào đã khởi công và quá ngày dự kiến hoàn thành' in text.lower()
    with toan_quyen.application.app_context():
        from app.modules.dashboard.service import lay_so_lieu_tong_quan
        so_lieu = lay_so_lieu_tong_quan()
        assert so_lieu['the_so_lieu']['so_hang_muc_qua_han'] == 0
        assert so_lieu['the_so_lieu']['so_hang_muc_dang_thi_cong'] == 0
        assert so_lieu['bieu_do']['hang_muc_theo_trang_thai']['Chưa xác định'] == 1
