...
def hang_muc_tre_tien_do():
    """Hạng mục ĐÃ khởi công (`NgayKhoiCong` không rỗng và đã qua/đúng hôm
    nay) NHƯNG `ThoiGianHoanThanhDuKien` cũng đã qua ngày hôm nay - dùng
    ...
    LỖI CŨ đã sửa: trước đây chỉ lọc theo `ThoiGianHoanThanhDuKien < hôm
    nay`, không quan tâm `NgayKhoiCong` - nên 1 hạng mục CHƯA khởi công
    (ngày khởi công còn ở tương lai, hoặc chưa nhập) nhưng lỡ có ngày dự
    kiến hoàn thành nằm trong quá khứ vẫn bị liệt kê nhầm vào danh sách
    "trễ tiến độ", dù thực tế còn chưa bắt đầu thi công thì không thể gọi
    là "trễ". Giờ chỉ tính "trễ tiến độ" cho hạng mục ĐÃ THỰC SỰ khởi công."""
    return HangMuc.query.filter(
        HangMuc.NgayKhoiCong.isnot(None),
        HangMuc.NgayKhoiCong <= date.today(),
